...
GITHUB_TOKEN = os.getenv("GITHUB_TOKEN")              # 🔐 GitHub Token من https://github.com/settings/tokens
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            ...
            async with session.get(
                ...
                if apps_data.get('apps'):
                    ...
                    # طباعة تفاصيل التطبيق للتشخيص
                    print(f"🔍 محتوى التطبيق: {list(app.keys())}")
                            ...
                            async with session.get(
                                ...
                                if all_svc_response.status == 200:
                                    ...
                                    if app_services:
                                        ...
                                        # محاولة أخيرة: استخدام app_id كـ service_id (أحياناً يكونان متشابهين)
                                        print("🔍 محاولة أخيرة: استخدام معرف التطبيق كمعرف الخدمة")
...
async def update_bot(event, repo, ups_rem, ac_br):
    ...
    try:
        print("🔄 جاري سحب التحديثات...")
        ups_rem.pull(ac_br)
    except GitCommandError:
        print("⚠️ تنفيذ reset --hard لحل التعارضات...")
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    ...
    try:
        print("🔄 جاري جلب التحديثات من upstream...")
        ups_rem.fetch(ac_br)
        repo.git.reset("--hard", "FETCH_HEAD")
        ...
        if GITHUB_TOKEN and UPSTREAM_REPO_URL:
            ...
            if UPSTREAM_REPO_URL.startswith("https://github.com/"):
                ...
                print(f"🔑 استخدام GitHub token للمصادقة")
            else:
                auth_url = UPSTREAM_REPO_URL
        else:
            auth_url = UPSTREAM_REPO_URL
            print("⚠️ لا يوجد GitHub token - قد تفشل عملية Push")
        ...
        print(f"🚀 جاري رفع التحديثات للـ repository...")
        ...
        try:
            ...
            print(f"❌ فشل في رفع التحديثات: {push_error}")
    ...
    print("🔄 جاري إعادة نشر الخدمة على Koyeb...")
    ...
    print("✅ تمت العملية بنجاح، جاري إعادة تشغيل البوت...")
...
# تأكد من أن client متاح في النطاق
# استبدل 'client' باسم متغير البوت الخاص بك
...
async def update_command(event):
    ...
    # تحذير إذا لم يكن هناك GitHub token
    if not GITHUB_TOKEN:
        print("⚠️ تحذير: لا يوجد GITHUB_TOKEN - قد تفشل عملية Push للـ repository الخاص")
    ...
    await loading_msg.edit(
        ...
        f"**✅ التطبيق:** `{KOYEB_APP_NAME}`\n"
        f"**✅ الخدمة:** `{KOYEB_SERVICE_ID[:20]}...`\n"
        f"**✅ المستودع:** `{UPSTREAM_REPO_URL.split('/')[-1]}`\n\n"
        "**🔧 جاري تحضير التحديث...**"
    )
...
"""
🔐 المتغيرات المطلوبة في Koyeb Environment Variables:

1. KOYEB_API_TOKEN
   - احصل عليه من: https://app.koyeb.com/account/api
   - مطلوب: نعم
   - مثال: koyeb_xxxxxxxxxxxxxxxxxxxxx

2. GITHUB_TOKEN  
   - احصل عليه من: https://github.com/settings/tokens
   - مطلوب: نعم (للـ repositories الخاصة)
   - صلاحيات مطلوبة: repo
   - مثال: ghp_xxxxxxxxxxxxxxxxxxxxx

3. UPSTREAM_REPO_URL
   - رابط الـ repository الخاص بك
   - مطلوب: نعم  
   - مثال: https://github.com/username/bot-name.git

4. UPSTREAM_REPO_BRANCH
   - اسم البرانش
   - افتراضي: main
   - مثال: main أو master

📋 خطوات الإعداد في Koyeb:
1. اذهب لـ Koyeb Dashboard
2. اختر تطبيقك → Settings
3. اضغط Environment Variables
4. أضف المتغيرات أعلاه
5. 🔒 فعل "Secret" للتوكنات
6. احفظ التغييرات

🚀 الاستخدام:
- اكتب: .تحديث البوت
- انتظر 4-5 دقائق
- سيتم إعادة تشغيل البوت تلقائياً

⚠️ ملاحظات هامة:
- لا تشارك التوكنات مع أحد
- استخدم صلاحيات محدودة
- جدد التوكنات دورياً
- احتفظ بنسخة احتياطية من الإعدادات
"""

print("✅ تم تحميل نظام تحديث البوت بنجاح!")
print("📋 للاستخدام: .تحديث البوت")
print("🔧 تأكد من إعداد المتغيرات في Koyeb Environment Variables")
