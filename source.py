...
async def download_and_send_audio(event):
    ...
    try:
        ...
        file_size = len(video_clip)  # طول الملف بالمللي ثانية
        if file_size > 300000:  # إذا كان الملف أكبر من 5 دقائق (300000 مللي ثانية)
            bitrate = "64k"  # معدل بت منخفض للملفات الكبيرة
        else:
            bitrate = "192k"  # معدل بت أعلى للملفات الصغيرة
        ...
        # إرسال الملف الصوتي
        await client.send_file(event.chat_id, audio_path, caption=f"**⌔╎البحث :** `{artist} - {video_title}`", supports_streaming=True)

        # تنظيف الملفات المؤقتة
        os.remove(download_path)
        os.remove(audio_path)

        await event.edit("**تم إرسال الملف الصوتي بنجاح! ✅**")
...
async def download_and_send_video(event):
    ...
    if not input_url:  # إذا لم يكن هناك رابط في الرسالة أو الرد
        await event.edit("**- ارسـل (`.يوت`) + رابـط او بالـرد ع رابـط**")
        return
    ...
    try:
        ...
        ydl_opts = {
            'format': 'best',  # لتحميل أفضل جودة متاحة
            'outtmpl': 'downloaded_video.%(ext)s',  # تحديد اسم الملف المُحمَّل
        ...
        await event.edit(f"**تم التحميل بنجاح! ✅\nجاري إرسال الفيديو: {video_title}**")

        # إرسال الفيديو
        await client.send_file(
            event.chat_id,
            video_file,
            caption=f"**📹 عنوان الفيديو: {video_title}**",
            supports_streaming=True  # يسمح بتشغيل الفيديو مباشرة من داخل التليجرام
        )

        # تنظيف الملفات المؤقتة
        os.remove(video_file)

        await event.edit("**تم إرسال الفيديو بنجاح! ✅**")

    except Exception as e:
        await event.edit(f"**⚠️ حدث خطأ أثناء التحميل:** {str(e)}")
...
async def download_and_send_tiktok(event):
    ...
    if not input_url:  # إذا لم يكن هناك رابط في الرسالة أو الرد
        await event.edit("**- ارسـل (`.تيك`) + رابـط او بالـرد ع رابـط**")
        return

    await event.edit("**جاري تحميل فيديو تيك توك...**")

    try:
        # إعدادات yt-dlp لتحميل فيديو من TikTok مع تحويله إلى تنسيق مدعوم باستخدام FFmpeg
        ydl_opts = {
            'format': 'best',  # لتحميل أفضل جودة متاحة
            'outtmpl': 'tiktok_video.%(ext)s',  # تحديد اسم الملف المُحمَّل
            'noplaylist': True,  # عدم تحميل قوائم التشغيل
            'quiet': True,
            'socket_timeout': 60,  # زيادة مهلة الاتصال إلى 60 ثانية
            'postprocessors': [{  # تحويل الفيديو باستخدام FFmpeg إلى H.264 و AAC
                'key': 'FFmpegVideoConvertor',
                'preferedformat': 'mp4',  # التحويل إلى MP4
            }],
        }
        ...
        with YoutubeDL(ydl_opts) as ydl:
            info = ydl.extract_info(input_url, download=True)
            video_title = info.get('title', 'فيديو بدون عنوان')
            video_file = ydl.prepare_filename(info).replace('.webm', '.mp4')  # تأكد من تحويل الامتداد إلى mp4
...
async def download_and_send_instagram(event):
    ...
    if not input_url:  # إذا لم يكن هناك رابط في الرسالة أو الرد
        await event.edit("**- ارسـل (`.انستا`) + رابـط او بالـرد ع رابـط**")
        return

    await event.edit("**جاري تحميل الفيديو من إنستجرام...**")

    try:
        # إعدادات yt-dlp لتحميل فيديو من إنستغرام مع الكوكيز
        ydl_opts = {
            'format': 'best',  # لتحميل أفضل جودة متاحة
            'outtmpl': 'instagram_video.%(ext)s',  # تحديد اسم الملف المُحمَّل
            'noplaylist': True,  # عدم تحميل قوائم التشغيل
            'quiet': True,
            'cookiefile': 'cookies.txt',  # إضافة ملف الكوكيز الذي قمت باستخراجه
        }
...
async def download_and_send_pinterest(event):
    ...
    if not input_url:
        await event.edit("**⚠️ يُرجى إدخال رابط Pinterest بعد الأمر أو الرد على رابط**")
        return

    await event.edit("**⏳ جاري معالجة الرابط...**")
    ...
    try:
        ...
        await event.edit("**✅ تم التحميل بنجاح! جاري الإرسال...**")

        # إرسال الملف
        await event.client.send_file(
            event.chat_id,
            filename,
            caption=f"**📌 {info.get('title', 'محتوى Pinterest')}**",
        ...
        await event.edit(f"**❌ فشل التحميل:**\n`{str(e)}`\n\nجرب رابطًا آخر أو تأكد من صحة الرابط")
...
# قائمة المستخدمين الذين يتم تقليدهم
imitated_users = set()
